...
function test_id(x) {
  return Primitive_object.equal(List.map(x, id), x);
}

function test_compose(xs) {
  let f = x => x % 2;
  let g = x => x - 1 | 0;
  return Primitive_object.equal(List.map(xs, extra => (extra - 1 | 0) % 2), List.map(List.map(xs, g), f));
}
...
function MonoidUtils(M) {
  ...
  let concat = xs => List.reduce(xs, empty, $less$plus$great);
  return {
    empty: empty,
    append: append,
...
function apply(fs, xs) {
  return List.flat(List.map(fs, f => List.map(xs, x => f(x))));
}
...
function $less$star(x, y) {
  return apply(List.map(x, $$const), y);
}

function $star$great(x, y) {
  return apply(List.map(x, param => (y => y)), y);
}

function liftA2(f, x, y) {
  return apply(List.map(x, f), y);
}
...
let quotes = apply(apply(apply(List.map({
  hd: 1,
  tl: {
    hd: 2,
    tl: {
...
let __x$20 = traverse$2(x => Math.imul(x, x), {
  TAG: "Node",
  _0: "Leaf",
  _1: 3,
  _2: {
...
function TestTraversableNat(T2, A1, A2, MT) {
  ...
  let test = (f, param, x) => {
    let t = param.t;
    return Primitive_object.equal(t(T1.traverse(f, x)), T2$1.traverse(extra => t(f(extra)), x));
  };
  return {
    T1: T1,
    T2: T2$1,
...
function partial_arg(param, param$1) {
  let T1 = param$1(IdApplicative);
  let T2 = param$1(param);
  let test = (f, param, x) => {
    let t = param.t;
...
let TTN = partial_arg(OptionApplicative, ListTraversable);
...
function partial_arg$1(param, param$1) {
  let pure$3 = x => pure(param.pure(x));
  let map$5 = f => (extra => map(extra => param.map(f, extra), extra));
  let apply$3 = (f, x) => {
    let map$6 = map;
    let apply$4 = apply;
    let $less$$great = (f, xs) => map$6(f, xs);
    let f$1 = $less$$great(fs => (xs => param.apply(fs, xs)), f);
    return apply$4(f$1, x);
  };
  let AC = {
    map: map$5,
    pure: pure$3,
  ...
  let TF = param$1(ListApplicative);
  let TG = param$1(param);
  let TC = param$1(AC);
  let test = (f, g, x) => Primitive_object.equal(map(__x => TG.traverse(g, __x), TF.traverse(f, x)), TC.traverse(__x => {
    let __x$1 = f(__x);
    return map(g, __x$1);
  }, x));
  return {
...
let TTCL = partial_arg$1(OptionApplicative, ListTraversable);

let prim = TTCL.test(x => ({
  hd: x,
  tl: {
    hd: x + 1 | 0,
    tl: /* [] */0
